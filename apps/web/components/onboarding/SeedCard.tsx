...
export function SeedCard({ movie, rating, onRate }: SeedCardProps) {
  ...
  return (
    <div className={cn("group relative aspect-[2/3] overflow-hidden rounded bg-[var(--surface)] bg-cover bg-center transition", rating === 1 && "ring-2 ring-[#4ADE80]", rating === -1 && "opacity-40")} style={{ backgroundImage: poster ? `url(${poster})` : undefined }}>
